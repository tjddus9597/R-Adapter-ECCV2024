...
from clip.model import build_model, convert_weights
...
def load(
    ...
    if not jit:
        try:
            model = build_model(state_dict or model.state_dict(), peft_config=peft_config, precision=precision).to(device)
        except KeyError:
            sd = {k[7:]: v for k,v in state_dict["state_dict"].items()}
            model = build_model(sd, precision=precision).to(device)
