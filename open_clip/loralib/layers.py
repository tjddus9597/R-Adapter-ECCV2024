...
class StochasticAdapter(nn.Module):
    def __init__(self, embed_dim, r=64, init_value=0.1, scale_train=False, drop_path=0.5):
        super().__init__()
        self.embed_dim = embed_dim
        self.r = r
        ...
        self.u_init = 'zero'
        self.d = tensor_prompt(2, self.embed_dim, self.r, init=self.d_init)
        self.u = tensor_prompt(2, self.r, self.embed_dim, init=self.u_init)
        self.s = nn.Parameter(init_value * torch.ones(2), requires_grad = self.scale_train)
            
        self.drop_paths1 = DropPath(drop_path)
        self.drop_paths2 = DropPath(drop_path)
                    
    def forward(self, x, f):
        x = torch.einsum('bnd,ds->bns', x, self.d[f])
        x = self.non_linear(x)
        x = torch.einsum('bns,sd->bnd', x, self.u[f])
        x = x.mul_(self.s[f])
        
        if f == 0:
            x = self.drop_paths1(x)
        else:
            x = self.drop_paths2(x)
