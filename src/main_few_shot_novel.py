...
def main(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % 10 == 0 and epoch > 0:
            ...
            ##########################################################################################################
            num_base_classes = len(dataset.get_classname('base'))
            classification_head_new = None
            if is_master(args):
                test_classnames = dataset.get_classname('base')
                ...
                test_dataloader = dataset.test_loader
                results = eval_single_dataloader(model, test_dataloader, args, classification_head_new, split='base', num_base_classes=num_base_classes)
                if 'top1' in results:
                    base_acc = results['top1']
                    if logging != None:
                        logging.info(
                            f"{args.train_dataset} Base Top-1 accuracy: {base_acc:.4f}")
                    epoch_stats[args.train_dataset + " Base Accuracy"] = round(base_acc, 4)

            ##########################################################################################################
            classification_head_new = None
            if is_master(args):
                test_classnames = dataset.get_classname('new')
                classification_head_new = get_zeroshot_classifier_fewshot(args, test_classnames, model.module.model)
                classification_head_new = classification_head_new.cuda()

                test_dataloader = dataset.test_loader
                results = eval_single_dataloader(model, test_dataloader, args, classification_head_new, split='new', num_base_classes=num_base_classes)
                if 'top1' in results:
                    new_acc = results['top1']
                    if logging != None:
                        logging.info(
                            f"{args.train_dataset} New Top-1 accuracy: {new_acc:.4f}")
                    epoch_stats[args.train_dataset + " New Accuracy"] = round(new_acc, 4)


            if is_master(args):
                H_acc = 2 * (base_acc * new_acc) / (base_acc + new_acc) 
                logging.info(
                    f"{args.train_dataset} Harmonic Top-1 accuracy: {H_acc:.4f}")
                epoch_stats[args.train_dataset + " Harmonic Accuracy"] = round(H_acc, 4)
