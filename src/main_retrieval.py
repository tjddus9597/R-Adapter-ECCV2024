...
from src.models.eval_retrieval import evaluate
from src.models.eval import get_autocast, get_cast_dtype
...
def main(args):
    ...
    num_batches = len(ft_dataloader.dataset) // args.world_size
    model = model.cuda()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_one_epoch(model, ft_dataloader, clip_loss_fn, epoch, optimizer, scaler, scheduler, args)
        completed_epoch = epoch + 1

        # Evaluate
        args.current_epoch = epoch
        args.eval_datasets = ['coco', 'f30k']
        if epoch >= 0:
            adapt_utils.Rep_AdaptWeight(model.module.model, args)
        
            eval_results = evaluate(model, args, clip_encoder.val_preprocess, epoch_stats, logging)
...
def train_one_epoch(model, ft_dataloader, loss, epoch, optimizer, scaler, scheduler, args):
    ...
    for i in range(num_batches):
        ...
        if is_master(args) and i % 100 == 1:
            ...
            samples_per_epoch = len(ft_dataloader.dataset)
