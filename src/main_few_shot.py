...
sys.path.append('/home/tjddus9597/R-Adapter')
...
def main(args):
    ...
    log_filename = None
    if is_master(args):
        os.makedirs("expt_logs/"+ "16_shot/" +args.exp_name, exist_ok=True)
        args.save = "expt_logs/"+ "16_shot/" + args.exp_name + "/" + "_BS" + str(
            args.batch_size) + "_WD" + str(args.wd) + "_LR" + str(args.lr) + "_run" + str(args.run)
        os.makedirs("expt_logs/" + "16_shot/" + args.exp_name, exist_ok=True)
        logging_path = "expt_logs/" +"16_shot/" +  args.exp_name + "/" + "_BS" + str(
            args.batch_size) + "_WD" + str(args.wd) + "_LR" + str(args.lr) + "_run" + str(args.run)
    ...
    sampler = dataset.get_train_sampler(True)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        args.eval_datasets = ['ImageNet', 'ImageNetV2','ImageNetR','ImageNetSketch','ImageNetA']
...
def train_one_epoch(model, dataset, ft_dataloader, loss, epoch, optimizer, scaler, scheduler, args):
    ...
    for i in range(num_batches):
        ...
        labels = batch['labels']
        texts = ['a photo of a ' + class_names[labels[i]] for i in range(len(labels))]
