...
def ce_ablation(args, clip_encoder, classification_head, logger):
    ...
    logger.info('Fine-tuning Using R-Adapter')
    model = clip_encoder
    input_key = 'images'
    ...
    for epoch in range(0, args.epochs):
        ...
        for i, batch in enumerate(data_loader):
            ...
            if i % print_every == 0:
                ...
                logger.info(
                    ...
                    f"ID R-Adapter: {xent_loss.item():.4f}")
        ...
        logger.info(f"Avg ID R-Adapter : {id_ce_loss_avg:.4f}")
        epoch_stats['Avg ID R-Adapter Loss'] = round(id_ce_loss_avg, 4)
